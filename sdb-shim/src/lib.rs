...
static mut REAL_PTHREAD_MUTEX_UNLOCK: Option<unsafe extern "C" fn(*mut pthread_mutex_t) -> i32> = None;

use fast_log::config::Config;

unsafe fn init() {
    INIT.call_once(|| {
        fast_log::init(Config::new().file("lock.log").chan_len(Some(1000))).unwrap();

        let lib = Library::new("libpthread.so.0").expect("Failed to load libpthread");

        let lock_func: libloading::Symbol<unsafe extern "C" fn(*mut pthread_mutex_t) -> i32> =
        ...
        let unlock_func: libloading::Symbol<unsafe extern "C" fn(*mut pthread_mutex_t) -> i32> =
            lib.get(b"pthread_mutex_unlock").expect("Failed to load pthread_mutex_lock symbol");
...
pub unsafe extern "C" fn pthread_mutex_lock(mutex: *mut pthread_mutex_t) -> i32 {
    ...
    log::info!("[lock][acquire]: thread={}, lock_addr={}", tid, mutex as u64);

    if let Some(real_pthread_mutex_lock) = REAL_PTHREAD_MUTEX_LOCK {
        let ret = real_pthread_mutex_lock(mutex);
        log::info!("[lock][acquired]: thread={}, lock_addr={}", tid, mutex as u64);
        ret
    } else {
